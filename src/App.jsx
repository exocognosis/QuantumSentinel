...
function Scan({ scans, setScans, setActive }) {
  ...
  async function startScan() {
    setError(""); setResultNote(""); setCompleted(false); setFailed(false); setLastResult(null); setRunning(true); setProgress(12);
    try {
      let request;
      ...
      else if (mode === "device") request = { mode: "device", scope: deviceScope, ports: devicePorts.split(/[\s,;]+/).filter(Boolean).map(Number).slice(0,8), timeoutMs: boundedTimeout };
      else request = { mode: "discovery", hosts: target.split(/[\s,;]+/).filter(Boolean).slice(0, Math.max(1, Math.min(16, Number(targetLimit) || 12))), ports: networkPorts.split(/[\s,;]+/).filter(Boolean).map(Number).slice(0,8), concurrency: Math.max(1,Math.min(8,Number(concurrency)||4)), timeoutMs: boundedTimeout };
      ...
      const noReachableService = ["discovery", "device"].includes(job.type) && job.result?.summary?.completedCount === 0;
  ...
  const modeConfig = {
    public: {
      title: "Public TLS posture scan", subtitle: "Observe the cryptography presented by one public TLS endpoint.",
      steps: ["Connect to TLS endpoint", "Inspect presented certificate", "Classify observed cryptography", "Record evidence & limitations"],
      scope: "The TLS version, cipher, forward-secrecy signal, and leaf certificate presented by one public endpoint. It does not inspect internal systems or determine organization-wide readiness.",
      observations: [[KeyRound,"TLS negotiation","Negotiated TLS version, cipher, and forward-secrecy signal."],[FileText,"Presented certificate","Leaf certificate issuer, algorithm, key size, expiry, and fingerprint."],[Sparkles,"Cryptographic posture","Observed RSA, ECC, hybrid, or post-quantum evidence."],[Clock3,"Exposure signal","Public-key evidence relevant to harvest-now-decrypt-later exposure."]],
    },
    device: {
      title: "Local TLS service check", subtitle: "Test the selected service port on this machine’s loopback addresses.",
      steps: ["Probe local loopback addresses", "Test selected TCP port", "Attempt TLS where reachable", "Record local service evidence"],
      scope: "Whether localhost and 127.0.0.1 accept TCP or TLS on the selected port, plus TLS evidence when available. It does not inspect files, installed software, keys, packages, or operating-system cryptography.",
      observations: [[Laptop,"Loopback targets","Tests localhost and 127.0.0.1 only."],[Network,"Port reachability","Records whether the selected TCP port accepts a connection."],[KeyRound,"Local TLS service","Attempts TLS and reads presented cryptography when available."],[CircleHelp,"Bounded evidence","Does not inventory device files, applications, keys, or every port."]],
    },
    network: {
      title: "Authorized network TLS discovery", subtitle: "Test an explicit list of authorized hosts on one selected service port.",
      steps: ["Validate authorized target list", "Test TCP reachability", "Attempt TLS on reachable hosts", "Summarize per-host evidence"],
      scope: "TCP reachability and available TLS evidence for the listed hosts on one selected port, up to 16 targets. It does not discover unknown devices, sweep port ranges, or inspect services beyond that scope.",
      observations: [[Network,"Authorized hosts","Tests only the hostnames or IP addresses explicitly entered."],[Activity,"TCP reachability","Records reachable and unreachable targets on the selected port."],[KeyRound,"TLS availability","Attempts TLS and captures endpoint cryptography where supported."],[FileText,"Per-host summary","Separates TLS evidence, TCP-only observations, and failures."]],
    },
  };
  ...
  const ModeIcon = mode === "public" ? Globe2 : mode === "device" ? Laptop : Network;
  const discoverySummary = lastResult?.summary;
  const completedSummary = mode === "public"
    ? [observedProtocol, observedAlgorithm, observedPfs===true?"forward secrecy observed":observedPfs===false?"forward secrecy not observed":null].filter(Boolean).join(" · ")
    : discoverySummary ? `${discoverySummary.completedCount} of ${discoverySummary.targetsScanned} targets returned service evidence · ${discoverySummary.failedCount} unreachable` : "Bounded service evidence recorded";
  const idleMeaning = mode === "public"
    ? ["This is the outside-in starting point", "A public TLS scan shows what one internet-facing endpoint presents. It cannot see internal PKI, VPNs, databases, code signing, stored data, or migration governance."]
    : mode === "device"
      ? ["This is a bounded loopback service check", "It tests two local names on one port. It does not inventory applications, cryptographic libraries, stored keys, files, or other device ports."]
      : ["This is a bounded host-and-port discovery", "It tests only the entered hosts on one port. It does not discover unknown devices, sweep networks, or inspect every service."];
  const selectMode = id => { setMode(id); setTarget(id==="public"?"":id==="device"?"Local machine":"10.0.0.1, 10.0.0.2"); setRunning(false); setCompleted(false); setFailed(false); setLastResult(null); setProgress(0); setError(""); setResultNote(""); };
  return <>
    <PageTitle title={currentMode.title} subtitle={currentMode.subtitle} />
    <section className="scan-grid">
      <article className="card scan-composer"><div className="mode-tabs">{modes.map(([id,Icon,label])=><button className={mode===id?"active":""} onClick={()=>selectMode(id)} key={id}><Icon/>{label}</button>)}</div><div className="target-input"><Search/><input value={target} placeholder={mode==="public"?"For example www.google.com":""} onChange={e=>setTarget(e.target.value)} disabled={mode==="device"} aria-label="Scan target"/></div><div className="scan-actions"><button className={`secondary ${advancedOpen?"open":""}`} aria-expanded={advancedOpen} onClick={()=>setAdvancedOpen(open=>!open)}><Settings2/>Advanced options<ChevronDown/></button><button className="primary" disabled={running || !target.trim()} onClick={startScan}><Play/>{running?"Scanning…":"Start scan"}</button></div>{advancedOpen&&<div className="advanced-panel mode-advanced">{mode==="public"?<><label><span>TLS service port</span><input aria-label="TLS service port" type="number" min="1" max="65535" value={port} onChange={e=>setPort(e.target.value)}/><small>443 is standard HTTPS.</small></label><label><span>External connection timeout</span><select aria-label="External connection timeout" value={timeoutMs} onChange={e=>setTimeoutMs(e.target.value)}><option value="1000">1 second</option><option value="2500">2.5 seconds</option><option value="5000">5 seconds</option><option value="10000">10 seconds</option></select><small>Allows for internet latency.</small></label></>:mode==="device"?<><label><span>Loopback scope</span><select aria-label="Loopback scope" value={deviceScope} onChange={e=>setDeviceScope(e.target.value)}><option value="both">localhost + 127.0.0.1</option><option value="localhost">localhost only</option><option value="ipv4">127.0.0.1 only</option></select><small>Never leaves this machine.</small></label><label><span>Local service ports</span><input aria-label="Local service ports" value={devicePorts} onChange={e=>setDevicePorts(e.target.value)}/><small>Comma-separated; maximum 8 ports.</small></label><label><span>Local connection timeout</span><select aria-label="Local connection timeout" value={timeoutMs} onChange={e=>setTimeoutMs(e.target.value)}><option value="250">0.25 seconds</option><option value="1000">1 second</option><option value="2500">2.5 seconds</option></select><small>Local services should respond quickly.</small></label></>:<><label><span>Authorized service ports</span><input aria-label="Authorized service ports" value={networkPorts} onChange={e=>setNetworkPorts(e.target.value)}/><small>Comma-separated; maximum 8 ports.</small></label><label><span>Per-host timeout</span><select aria-label="Per-host timeout" value={timeoutMs} onChange={e=>setTimeoutMs(e.target.value)}><option value="1000">1 second</option><option value="2500">2.5 seconds</option><option value="5000">5 seconds</option></select><small>Applied to each host and port pair.</small></label><label><span>Maximum targets</span><input aria-label="Maximum targets" type="number" min="1" max="16" value={targetLimit} onChange={e=>setTargetLimit(e.target.value)}/><small>Safety limit: 16 hosts.</small></label><label><span>Concurrent probes</span><select aria-label="Concurrent probes" value={concurrency} onChange={e=>setConcurrency(e.target.value)}><option value="1">1 — slowest</option><option value="2">2</option><option value="4">4 — recommended</option><option value="8">8 — fastest</option></select><small>Bounds network load; maximum 8.</small></label></>}<div className="advanced-note"><ShieldCheck/><span><b>{mode==="public"?"External endpoint evidence":mode==="device"?"Local runtime and loopback evidence":"Bounded authorized network evidence"} is saved automatically.</b><small>Only successful cryptographic observations contribute to readiness evidence.</small></span></div></div>}<p className="consent"><Shield/>Only scan systems you own or are authorized to test.</p>{error&&<p className="error">{error}</p>}</article>
      <article className={`card active-scan ${failed?"scan-failed":""}`}><div className="scan-target"><span className={`metric-icon ${failed?"red":"blue"}`}>{failed?<ShieldAlert/>:<ModeIcon/>}</span><b>{target || "No target selected"}</b><span className={`status-pill ${failed?"failed":""}`}>{running?"Active scan":failed?"Failed":completed?"Completed":"Ready"}</span></div><div className="progress-layout"><div className="progress-ring" style={{"--progress":`${progress*3.6}deg`}}><div><strong>{failed?"—":`${progress}%`}</strong><span>{failed?"Failed":running?"Complete":completed?"Complete":"Ready"}</span></div></div><div className="steps">{scanSteps.map((s,i)=><div className={failed&&i===0?"failed":(progress > (i+1)*22)?"done":progress > i*22?"current":""} key={s}><i>{progress > (i+1)*22&&!failed?<Check/>:failed&&i===0?"!":null}</i><span>{s}</span><small>{failed&&i===0?"Failed":failed?"Not run":progress > (i+1)*22?"Done":progress > i*22?"Active":"Pending"}</small></div>)}</div></div><p className="scan-scope"><CircleHelp/><span><b>What this proves:</b> {currentMode.scope}</span></p><div className="scan-footer"><span>{failed?<ShieldAlert/>:<Clock3/>}{running?"Probe in progress":failed?(resultNote||"No evidence collected"):completed?(resultNote||"Scan completed"):"Choose a target to begin"}</span>{running&&<button className="secondary" onClick={()=>setRunning(false)}>Cancel scan</button>}</div></article>
      <RecentScans scans={scans} onRescan={(scan)=>{setTarget(scan.targetLabel);setMode(scan.type==="local"?"device":"public");}} />
      <article className="card check-card"><div className="card-heading"><span><ShieldCheck/>What we observe</span></div>{currentMode.observations.map(([Icon,t,d])=><div className="check-row" key={t}><span><Icon/></span><div><b>{t}</b><small>{d}</small></div></div>)}<button className="text-link" onClick={()=>setActive("Q-Day Readiness")}>Learn about scoring <ChevronRight/></button></article>
      ...
      <article className="insight scan-meaning"><span className={`metric-icon ${failed?"red":completed?"green":"blue"}`}>{failed?<ShieldAlert/>:completed?<ShieldCheck/>:<ModeIcon/>}</span><div><span className="eyebrow">What this {completed?"result":"scan"} means</span><h3>{failed?"No cryptographic conclusion can be drawn":completed?(mode==="public"?"One public endpoint observed—not the organization":mode==="device"?"Local service evidence collected—not a device inventory":"Authorized host evidence collected—not a network inventory"):idleMeaning[0]}</h3>{completed?<p><b>{completedSummary}</b><br/>{mode==="public"?"This describes only the endpoint scanned. Add authorized internal observations and your organization profile before treating it as a readiness signal.":mode==="device"?"This describes only loopback services on the selected port. A device cryptographic inventory requires separate filesystem, package, key, and application collectors.":"This describes only the entered hosts and selected port. Broader readiness requires approved asset inventory, additional services, and governance evidence."}</p>:failed?<p>The target returned no usable evidence within this scan’s stated bounds. Correct the target, port, or connection issue before using it in an assessment.</p>:<p>{idleMeaning[1]}</p>}</div><button className="primary" onClick={()=>{setMode(mode==="public"?"network":"public");setTarget(mode==="public"?"10.0.0.1, 10.0.0.2":"");setCompleted(false);setFailed(false);setLastResult(null)}}>{mode==="public"?"Next: authorized network":"Return to public scan"} <ChevronRight/></button></article>
    </section>
  </>;
}

function RecentScans({ scans, onRescan }) {
  const rows = scans.filter(scan => scan.status === "COMPLETED" && scan.result && !(["discovery","device"].includes(scan.type) && scan.result?.summary?.completedCount === 0)).filter((s,i,a)=>a.findIndex(x=>x.targetLabel===s.targetLabel)===i).slice(0,3);
...
export default function App() {
  ...
  const page = useMemo(() => {
    ...
    if (active === "Scan") return <Scan scans={scans} setScans={setScans} setActive={setActive}/>;
