...
function Scan({ scans, setScans }) {
  ...
  const scanSteps = mode === "public"
    ? ["Connect to TLS endpoint", "Inspect presented certificate", "Classify observed cryptography", "Record evidence & limitations"]
    : ["Connect to approved endpoint", "Inspect available certificate", "Classify observed cryptography", "Record evidence & limitations"];
  ...
  return <>
    ...
    <section className="scan-grid">
      ...
      <article className="insight scan-meaning"><span className={`metric-icon ${failed?"red":completed?"green":"blue"}`}>{failed?<ShieldAlert/>:completed?<ShieldCheck/>:<Sparkles/>}</span><div><span className="eyebrow">What this {completed?"result":"scan"} means</span><h3>{failed?"No cryptographic conclusion can be drawn":completed?"One public endpoint observed—not the organization":"This is the outside-in starting point"}</h3>{completed?<p><b>{[observedProtocol,observedAlgorithm,observedPfs===true?"forward secrecy observed":observedPfs===false?"forward secrecy not observed":null].filter(Boolean).join(" · ")}</b><br/>This evidence describes only the TLS endpoint scanned. Add authorized internal observations and your organization profile before treating it as a readiness signal.</p>:failed?<p>The target returned no usable TLS evidence. Correct the address or connection issue before using it in an assessment.</p>:<p>A public TLS scan shows what one internet-facing endpoint presents. It cannot see internal PKI, VPNs, databases, code signing, stored data, or migration governance.</p>}</div><button className="primary" onClick={()=>{setMode("network");setTarget("10.0.0.1, 10.0.0.2");setCompleted(false);setFailed(false);setLastResult(null)}}>{completed?"Next: internal evidence":"Scan authorized network"} <ChevronRight/></button></article>
    </section>
  </>;
}
